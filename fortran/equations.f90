    ...
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) :: dtauda, grhoa2, grhov_t(max_num_of_fluids)
    real(dl), dimension(:), allocatable :: grho_de
    integer :: i
    ...
    real(dl) dgrho_de(max_num_of_fluids), dgq_de(max_num_of_fluids), cs2_de
    ...
    do i = 1, State%CP%DarkEnergy%num_of_components
        grho = grho + grhov_t(i)
    end do
    ...
    real(dl) pir,adotoa
    real(dl) w_dark_energy_t(max_num_of_fluids)
