    ...
    private
    ! JVR - Changing the dark energy interface to accomodate multiple dark energy fluid components
    ! The number of components must be in the interface, so the skeleton subroutines can be made to return arrays.
    integer, public, parameter :: max_num_of_fluids = 4
    integer, public, parameter :: max_num_of_params = 5

    type, extends(TCambComponent) :: TDarkEnergyModel
        logical :: is_cosmological_constant = .true.
        integer :: num_perturb_equations = 0
        integer :: num_of_components = 1
    contains
    procedure :: Init
    ...
    real(dl), intent(in), dimension(max_num_of_fluids) :: grhov_t, w
    ...
    function TDarkEnergyEqnOfState_w_de(this, a) result(w)
    class(TDarkEnergyEqnOfState) :: this
    real(dl), dimension(max_num_of_fluids) :: w
    real(dl) :: al
    real(dl), intent(IN) :: a

    if(.not. this%use_tabulated_w) then
        w(1)= this%w_lam + this%wa*(1._dl-a)
    else
        al=dlog(a)
        if(al <= this%equation_of_state%Xmin_interp) then
            w(1)= this%equation_of_state%F(1)
        elseif(al >= this%equation_of_state%Xmax_interp) then
            w(1)= this%equation_of_state%F(this%equation_of_state%n)
        else
            w(1) = this%equation_of_state%Value(al)
        endif
    endif
    ...
    class(TDarkEnergyEqnOfState) :: this
    real(dl) :: grho_de(max_num_of_fluids), al, fint
    real(dl), intent(IN) :: a
    ...
    if(.not. this%use_tabulated_w) then
        ...
        if (this%wa/=0) then
            grho_de(1)=grho_de(1)*exp(-3. * this%wa * (1._dl - a))
        end if
    else
        if(a == 0.d0)then
